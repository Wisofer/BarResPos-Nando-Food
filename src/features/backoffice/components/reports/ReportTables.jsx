          ...
          <tbody className="divide-y divide-slate-100">
            {productosTopRows.length ? (
              productosTopRows.map((x, idx) => {
                ...
                return (
                  <tr key={idx} className={`group hover:bg-slate-50 transition-colors ${idx < 3 ? 'bg-amber-50/30' : ''}`}>
                    <td className="px-4 py-4 font-semibold text-slate-900">
                      {badge && <span className="mr-2 text-lg">{badge}</span>}
                      {x.producto || x.nombre || "—"}
                    </td>
                    <td className="px-4 py-4 text-slate-600 font-medium">{x.cantidad ?? x.unidades ?? 0}</td>
            ...
            {meserosRows.length ? (
              meserosRows.map((x, idx) => (
                <tr key={idx} className={`group hover:bg-slate-50 transition-colors ${idx === 0 ? 'bg-orange-50/50' : ''}`}>
                  ...
                  <td className="px-4 py-4 font-bold text-slate-700 text-right">{formatCurrency(x.promedioTicket ?? x.PromedioTicket ?? 0)}</td>
                </tr>
              ))
            ) : (
              <EmptyRow colSpan={4} />
                  ...
                  return (
                    <React.Fragment key={cierreId(row) ?? idx}>
                      ...
                      <tr className="bg-slate-50/50 border-b border-slate-100">
                        ...
                        <td colSpan={5} className="px-4 py-2 text-right text-xs text-slate-500">
                          <span className="mr-3 text-emerald-600/80 font-medium">Efectivo: {formatCurrency(row.totalEfectivo || row.TotalEfectivo || 0, currencySymbol)}</span>
                          <span className="mr-3 text-blue-600/80 font-medium">Tarjeta: {formatCurrency(row.totalTarjeta || row.TotalTarjeta || 0, currencySymbol)}</span>
                          <span className="text-purple-600/80 font-medium">Transferencia: {formatCurrency(row.totalTransferencia || row.TotalTransferencia || 0, currencySymbol)}</span>
                        </td>
                      </tr>
                    </React.Fragment>
                  );
          ...
          {movimientosRows.length ? (
            movimientosRows.map((x, idx) => (
              <tr key={idx} className="group hover:bg-slate-50 transition-colors">
                ...
                <td className="px-4 py-4">
                  <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${x.categoriaProducto === 'Cocina' ? 'bg-amber-100 text-amber-800' : 'bg-blue-100 text-blue-800'}`}>
                    {x.categoriaProducto || "—"}
                  </span>
                </td>
                <td className="px-4 py-4 text-slate-600">
                  <span className={`font-medium ${x.tipo === 'Entrada' ? 'text-emerald-600' : 'text-rose-600'}`}>
                    {x.tipo || "—"}
                  </span>
                  {x.subtipo && <span className="text-slate-400 text-xs ml-1">({x.subtipo})</span>}
                </td>
